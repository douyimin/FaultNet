...
import argparse
import segyio
from utils import prediction, write_data, create_out_dir
import os

parser = argparse.ArgumentParser(description='Prediction management')
parser.add_argument('--input', type=str, default=r'data/kerry.npy',
...
if __name__ == '__main__':
    ...
    if args.save_fault_cuboid:
        np.save(
            os.path.join(args.output_dir, os.path.split(args.input)[-1], os.path.split(args.input)[-1] + '_Fault.npy'),
